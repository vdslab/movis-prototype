...
import fd from "./seed-data/filmarks.json";
...
const filmarks = async () => {
  console.log("start");
  // await (fd as any).forEach(async (d, i) => {
  for (const d of fd as any) {
    const actors = [];
    for (const actor of d["actors"]) {
      const eActor = await prisma.actor.findFirst({
        where: {
          jfdbId: actor.id,
        },
      });
      ...
      if (eActor) {
        ...
        const nActor = await prisma.actor.create({
          data: {
            name: actor.name,
            jfdbId: actor.id,
          },
    ...
    console.log(`moive created id: ${movie.id}`);
    // console.log(`moive ${i} created id: ${movie.id}`);
  }
  // });
  console.log("end");
};
