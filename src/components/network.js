  ...
  export default function Graph(){
    ...
    const [selected, setSerected] = useState([]);//データだけ入る段階。
    ...
    useEffect(()=>{
      const data = {//いったん用 選択によってここを更新する処理が必要 data.を要らなくする propsにて
        nodes:[
          {id: '神木隆之介'} ,
    {id: '藤原竜也'} ,
    {id: '土屋太鳳'} ,
    ...
    {target: 'てらそままさき', source: '鈴村健一', weight: '1'} ,
      ...
      function firstSimuration(){
        //ここ参照https://wizardace.com/d3-forcesimulation-onlynode/
        const simulation = d3.forceSimulation()//ここで引力とかの設定を行う
          .force("collide", d3.forceCollide().radius(function (d ) { return d.r; }).iterations(10)) 
          .force("link", d3.forceLink().distance((d) => 1).id((d) => d.id))
      ...
      function ticked() {
          ...
          setNodes(data.nodes.slice()); //何で必要か？？
          setLinks(data.links.slice());
    ...
    return (
      <svg className="graph" width="1000" height="1000" >
        <g key="network">
            ...
            {nodes.map((node,i) => {
                return(
                    <circle key = {i} r="10"  cx={node.x} cy={node.y} onClick={()=> setSerected([...selected, node])}/> // [...ssa]フーガが言ってた奴のはず */}
          ...
          {nodes.map((node,i)=>{
              return(
                ...
                <text key = {i}  fill="red" x={node.x} y={node.y}>{node.id}</text>
              )
          })}
        {links.map((link,i) => {
            return(
